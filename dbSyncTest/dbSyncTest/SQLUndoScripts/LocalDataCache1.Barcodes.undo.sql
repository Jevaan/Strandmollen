...
IF @@TRANCOUNT > 0
ALTER TABLE [Barcodes] DROP CONSTRAINT [DF_Barcodes_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [Barcodes] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [Barcodes] DROP CONSTRAINT [DF_Barcodes_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [Barcodes] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[Barcodes_Tombstone]') and TYPE = N'U') 
   DROP TABLE [Barcodes_Tombstone];
